...
def draw_triangle(t, s, c):
    ...
    WHEN DONE, THE FOLLOWING TURTLE ATTRIBUTES ARE THE SAME
    AS IT STARTED:
...
def draw_spiral(w, side, ang, n, sp):
    ...
    assert is_valid_iteration(n), report_error('Invalid number of iterations', n)
    ...
    w.clear()
    t=tkturtle.Turtle(w, (0,0), "red", 0, sp)
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_length(side), report_error('Invalid length', side)
    assert is_valid_speed(sp), report_error('Invalid speed',sp)
    assert is_valid_iteration(k), report_error("Invalid # of sides", k)
    assert type(n)==int and n>=3, report_error("Invalid # of polygons", n)
    # HINT: w.clear() clears window.
    # HINT: set the turtle's visible attribute to False at the end.
    w.clear()
    t=tkturtle.Turtle(w, (0,0), "red", 90, sp)
...
def multi_polygons_helper(t, side, k, n, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('Inalid length', side)
    assert is_valid_speed(sp), report_error('Invalid speed',sp)
    assert is_valid_iteration(k), report_error("Invalid # of polygons", k)
    assert type(n)==int and n>=3, report_error("nInvalid # of sides", n)
...
def draw_polygon(t, side, n, sp):
    ...
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(side), report_error('Invalid side length',side)
    assert (type(n) == int and n >= 3), report_error('Invalid # of sides',n)
    assert is_valid_speed(sp), report_error('Invalid speed',sp)
...
def cantor(w, side, hght, d):
    ...
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(side), report_error('Invalid length',side)
    assert is_valid_length(hght), report_error('Invalid hight ',hght)
...
def cantor_helper(p, x, y, side, hght, d):
    ...
    if d==0:
        return fill_rect(p, x, y, side, hght)
    else:
        s1=x-1.0*side/3.0
        s2=x+1.0*side/3.0
        cantor_helper(p, s1, y-hght/4.0, side/3.0, hght/2.0, d-1)
        
        cantor_helper(p, s2, y-hght/4.0, side/3.0, hght/2.0, d-1)
        fill_rect(p, x, y+hght/4, side, hght/2.0)
...
def sierpinski(w, side, d):
    ...
    w.clear()
    p=tkturtle.Pen(w, (0,0), "magenta", 0)
    p.visible=False
...
def sierpinski_helper(p, x, y, side, d):
    ...
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_valid_length(side), report_error("Invalid length", side)
    assert is_valid_depth(d), report_error("Invlid depth", d)
    # HINT: Use fill_triangle instead of setting p's position directly
    if d==0:
        fill_triangle(p, side)
    else:
        sierpinski_helper(p, x+0, y+0, side/2.0, d-1)
        p.move( x+side/4.0, y+((side/2.0) * math.sqrt(0.75)))
        sierpinski_helper(p, x+side/4.0, y+(side/2.0) * math.sqrt(0.75), side/2.0, d-1)
        p.move(x+side/2.0, y+0)
        sierpinski_helper(p, x+side/2.0, y+0, side/2.0, d-1)
...
def branches(w, hght, d): 
    ...
    w.clear()
    t=tkturtle.Turtle(w, (0, -hght/2.0), "blue", 90, 0) 
...
def branches_helper(t, hght, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_turtlemode(t), report_error('Invalid turtle mode', t)
    assert is_valid_length(hght), report_error("Invalid length", hght)
    assert is_valid_depth(d), report_error("Invalid depth", d)
    ...
    if d==0:
        ...
        t.forward(hght/2.0)
        t.left(90)
        branches_helper(t, 0.5*hght, d-1)
        t.right(90)
        branches_helper(t, 0.5*hght, d-1)
        t.right(90)
        branches_helper(t, 0.5*hght, d-1)
