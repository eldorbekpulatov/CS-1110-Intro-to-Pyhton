...
def replace(thelist,a,b):
    ...
    if len(thelist)==0:
        ...
        x.append(thelist[0])
    return x+replace(thelist[1:], a, b)
...
def remove_dups(thelist):
    ...
    if len(thelist)==1:
        ...
        x.append(thelist[0])
    return x+remove_dups(thelist[1:])
...
def oddsevens(thelist):
    ...
    if thelist[0]%2==1:
        x.insert(0, thelist[0])
        
    else:
        y.insert(0, thelist[0])
        
    return x+oddsevens(thelist[1:])+y
...
def number_not(thelist, v):
    ...
    if thelist[0]==v:
        acc+=1
    return acc+numberof(thelist[1:],v)
